...
<head>
  ...
  .badge__nameblock { padding-top: 45mm; }
  .badge__name    { font-weight: 700; font-size: 18pt; font-family: 'NotoSans', sans-serif; }
  ...
  .badge__name .bottom-line {
    font-weight:400;
  }
  .badge__country { font-size: 16pt; margin-top: .5mm; }

  .badge__qrwrap   { margin: 4mm auto 0; width: 60mm; height: 60mm; }
  .badge__qrwrap img { width: 60mm; height: 60mm; display: block; margin: 0 auto; }

</style>
</head>
<body>

@foreach ($badges as $personnel)
    <div class="badge" style="background: {{ $personnel->bg_color ?? '#ffffff' }};">
        ...
        <div class="badge__nameblock">
            <div class="badge__name">
                <span class="name-line">{{ $personnel->line_1 }}</span>
                <span class="name-line">{{ $personnel->line_2 }}</span>
                <span class="bottom-line">{{ $personnel->line_3 }}</span>
            </div>
        </div>

        <div class="badge__groupband" style="background-color:{{$personnel->group_color}};">
      <p style="color:{{$personnel->group_text_color}}">{{ strtoupper($personnel->group_label) }}</p>
    </div>
    </div>
@endforeach
